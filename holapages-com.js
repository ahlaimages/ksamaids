function en_to_ar_number_conversion(en_number) {
	var ar_number = '';

	for (var i = 0; i < en_number.length; i++) {

		if (en_number[i] == '0') ar_number = ar_number + "٠";

		if (en_number[i] == '1') ar_number = ar_number + "١";

		if (en_number[i] == '2') ar_number = ar_number + "٢";

		if (en_number[i] == '3') ar_number = ar_number + "٣";

		if (en_number[i] == '4') ar_number = ar_number + "٤";

		if (en_number[i] == '5') ar_number = ar_number + "٥";

		if (en_number[i] == '6') ar_number = ar_number + "٦";

		if (en_number[i] == '7') ar_number = ar_number + "٧";

		if (en_number[i] == '8') ar_number = ar_number + "٨";

		if (en_number[i] == '9') ar_number = ar_number + "٩";

		if (en_number[i] == ':') ar_number = ar_number + ":";
		
		if (en_number[i] == 'am') ar_number = ar_number + "ص";
...
function showPrayTimes(lat, long, prayTimes) {
	...
	for (var i = 0; i < listLength; i++) {
		...
		if (currentWaqt == '') {
			if (moment().isBetween(startTime, endTime)) {
				...
				if($("input[name='lang']:checked").val() === 'ar'){
					nextPrayerTimeRemaining = en_to_ar_number_conversion(moment(nextPrayerTime.diff(moment())).utc().format('HH:mm:ss'));
					nextPrayerTimeName = listar[nextPrayerTimeNameIndex];
				}
		...
		if($("input[name='lang']:checked").val() === 'ar'){
			paryer_time = en_to_ar_number_conversion(prayerTimes[list[i].toLowerCase()]);
			waqt_name = listar[i];
		}
